...
static quill::Logger* initLogger(const std::string& logFilename = "app.log", const std::string& name = "app") {
  ...
  file_handler->set_pattern("[%(ascii_time)] #%(process).%(thread) %(filename)/%(function_name:):%(lineno:<8) [%(logger_name)] "
      "%(level_name) %(message)", "%D %H:%M:%S.%Qms %z", quill::Timezone::LocalTime);

  auto console_handler = quill::stdout_handler();
  console_handler->set_pattern("[%(ascii_time)] #%(process).%(thread) %(filename)/%(function_name:):%(lineno:<8) [%(logger_name)] "
      "%(level_name) %(message)", "%D %H:%M:%S.%Qms %z", quill::Timezone::LocalTime);
